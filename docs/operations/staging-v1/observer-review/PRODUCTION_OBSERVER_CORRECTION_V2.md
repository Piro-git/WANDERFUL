...
Reconciliation note: this document preserves the correction commit's review
history. The integrated contract is now defined by
`../observer-review-v2/PRODUCTION_OBSERVER_CAPABILITY_AND_POLICY_V2.md` and its
companion JSON. It has separate `staging_initialization` and
`production_admission` decisions; both remain typed `blocked`. The correction
improved the original adversarial result from 29/61 to 35/61, and the rebased V2
suite is 61/61 without treating unavailable platform evidence as success.
