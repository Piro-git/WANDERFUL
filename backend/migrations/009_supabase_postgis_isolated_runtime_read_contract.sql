-- Supabase PostGIS-isolation policy V2 replacement for historical migration 008.
-- This migration is mutually exclusive with 008. The approved V2 ledger applies
-- 001-007 followed by this file; it must never run after 008.
--
...
BEGIN
    ...
    IF owner_role.rolname IS NULL OR
       ...
       owner_role.rolbypassrls THEN
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'Supabase isolation migration 009 requires a NOLOGIN NOINHERIT least-privilege owner';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'Supabase isolation migration 009 owner must not inherit or SET ROLE to another role';
    END IF;
    ...
    IF application_schema IS NULL OR NOT EXISTS (
        ...
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'Supabase isolation migration 009 owner must own the application schema';
    END IF;

    IF application_schema <> 'trailmind_app' THEN
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'Supabase isolation migration 009 requires trailmind_app';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION USING
          ERRCODE = '55000',
          MESSAGE = 'PostGIS must already be installed directly in trailmind_gis';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WITH topology AS (
          ...
          UNION ALL
          SELECT relation.relowner FROM topology JOIN pg_catalog.pg_class relation
            ON relation.relnamespace = topology.namespace_oid
          UNION ALL
          SELECT type_record.typowner FROM topology JOIN pg_catalog.pg_type type_record
            ON type_record.typnamespace = topology.namespace_oid
          UNION ALL
          SELECT operator_record.oprowner FROM topology JOIN pg_catalog.pg_operator operator_record
            ON operator_record.oprnamespace = topology.namespace_oid
          UNION ALL
          SELECT operator_class.opcowner FROM topology JOIN pg_catalog.pg_opclass operator_class
            ON operator_class.opcnamespace = topology.namespace_oid
          UNION ALL
          SELECT operator_family.opfowner FROM topology JOIN pg_catalog.pg_opfamily operator_family
            ON operator_family.opfnamespace = topology.namespace_oid
          UNION ALL
          SELECT collation_record.collowner FROM topology JOIN pg_catalog.pg_collation collation_record
            ON collation_record.collnamespace = topology.namespace_oid
          UNION ALL
          SELECT conversion_record.conowner FROM topology JOIN pg_catalog.pg_conversion conversion_record
            ON conversion_record.connamespace = topology.namespace_oid
          UNION ALL
          SELECT configuration.cfgowner FROM topology JOIN pg_catalog.pg_ts_config configuration
            ON configuration.cfgnamespace = topology.namespace_oid
          UNION ALL
          SELECT dictionary.dictowner FROM topology JOIN pg_catalog.pg_ts_dict dictionary
        ...
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'PostGIS schema, extension, member ownership or write boundary is not the reviewed managed topology';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
          ERRCODE = '55000',
          MESSAGE = 'public PostGIS routines are forbidden by policy V2';
    END IF;

    IF NOT pg_catalog.has_schema_privilege(
         CURRENT_USER, 'trailmind_gis', 'USAGE'
       ) OR pg_catalog.has_schema_privilege(
         CURRENT_USER, 'trailmind_gis', 'CREATE'
       ) THEN
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'migration owner requires GIS USAGE without GIS CREATE';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'migration owner must not own the GIS schema or PostGIS routines';
    END IF;
    ...
    IF EXISTS (
        SELECT 1
         ...
         WHERE namespace.nspname = ANY(
           ARRAY[application_schema, 'trailmind_gis']::text[]
         )
           AND privilege.privilege_type = 'CREATE'
        ...
        RAISE EXCEPTION USING
          ERRCODE = '42501',
          MESSAGE = 'Supabase isolation migration 009 search path contains a writable untrusted schema';
    END IF;
END
$migration$;
...
SELECT pg_catalog.set_config(
  'search_path',
  'pg_catalog,trailmind_app,trailmind_gis,pg_temp',
  true
);

CREATE OR REPLACE FUNCTION trailmind_app.trailmind_runtime_outdoor_research_snapshot_context_v1(
    text,
    double precision,
    double precision
)
...
CREATE OR REPLACE FUNCTION trailmind_app.trailmind_runtime_outdoor_research_route_memberships_v1(
    uuid,
    text,
    double precision,
    double precision,
...
CREATE OR REPLACE FUNCTION trailmind_app.trailmind_runtime_outdoor_research_route_assertions_v1(
    uuid,
    uuid[],
    text[],
    integer
...
CREATE OR REPLACE FUNCTION trailmind_app.trailmind_runtime_outdoor_research_highlights_v1(
    uuid,
    text,
    double precision,
    double precision,
...
CREATE OR REPLACE FUNCTION trailmind_app.trailmind_runtime_outdoor_research_trail_access_candidates_v1(
    uuid,
    text,
    uuid[],
    double precision,
...
BEGIN
  ...
  FOREACH function_oid IN ARRAY expected
  LOOP
    IF NOT EXISTS (
      SELECT 1
      ...
      RAISE EXCEPTION USING
        ERRCODE = '42501',
        MESSAGE = 'runtime function owner, SECURITY DEFINER, search_path, or PUBLIC EXECUTE boundary is invalid';
    END IF;
  END LOOP;
END
$migration$;
