...
LEFT JOIN pg_catalog.pg_stat_ssl AS ssl
  ...
  pg_catalog.to_char(
    activity.query_start AT TIME ZONE 'UTC',
    'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
  ) AS query_start
FROM pg_catalog.pg_stat_activity AS activity
WHERE activity.pid <> $4::integer
  AND (
    activity.pid = $1::integer
    OR
    (
      activity.pid = $1::integer
      AND activity.application_name = $2::text
    ...
    OR activity.application_name LIKE 'trailmind_p1v2_auditor\\_%'
      ESCAPE '\\'
  )
ORDER BY activity.pid, activity.backend_start, activity.application_name`,
...
export function stagingPhase1V2ProductionAuditorConnectionContract() {
  ...
  return deepFreeze({
    applicationNamePattern: "^trailmind_p1v2_auditor_[a-f0-9]{32}$",
    channelBinding: "require",
    connectTimeoutSeconds: 10,
    database: policy.database,
    directPort: 5432,
    directHost: "db.mbvzwsrtqcrwhvykugcd.supabase.co",
    role: policy.role,
    sslMode: "verify-full",
    sslrootcertSha256:
      STAGING_PHASE1_V2_PRODUCTION_OBSERVER_POLICY.pins
        .auditorSslrootcertSha256,
