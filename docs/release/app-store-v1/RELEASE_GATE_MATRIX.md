...
Baseline: `21f8450c976252210edf03389dc1b682d2440450`
...
Exactly 25 of 50 applicable gates are proved: **50.0%**. A grouped row counts as one gate. Simulator or generic-device-build evidence does not complete an archive, physical-device, signed-distribution or external gate.

| Gate | Requirement | Classification | Current evidence or acceptance criterion | Blocker |
| --- | --- | --- | --- | --- |
| G-001 | Baseline, clean shared checkout and duplicate gate | proved | Audited application source is exact reachable commit `21f8450c976252210edf03389dc1b682d2440450`; the dedicated worktree was clean, and final origin advancement was confined to the non-overlapping Supabase migration-operator lane | — |
...
| G-005 | Launch appearance is complete and release-polished | proved | Forced-light production override is absent; adaptive light/dark/high-contrast theme tests and the deterministic light/dark/App Store visual matrix pass on the audited baseline | — |
...
| G-019 | Public privacy policy and in-app link | public-URL/legal | Native Privacy & data destination and strict fail-closed external-link slot are implemented; owner must publish and configure the real canonical HTTPS policy | ASV1-004 |
| G-020 | Public support URL and contact | public-URL/legal | Native Help & safety destination and strict fail-closed external-link slot are implemented; owner must publish a real page with monitored contact/legal details | ASV1-005 |
...
| G-035 | Accessibility common-task proof and labels | locally actionable | Labels/headings, adaptive contrast, Reduce Motion and accessibility XXXL route layouts are locally proved; actual VoiceOver focus handoff and Voice Control common tasks remain unproved | ASV1-016 |
...
| G-040 | Release verifier matches shipping identity | proved | Current verifier enforces production identity/version, nine false flags, four empty services, two empty public-link defaults, exact first/third-party privacy manifests, dSYM, signing/team/profile/entitlements, attribution and release composition; 46 adversarial cases plus stale recovery pass | — |
| G-041 | Built identity, flags, permissions and secret absence | proved | Current built products prove exact lane identities, nine false flags, empty service/public-link values, expected purpose strings, valid signatures/manifests and no prohibited first-party release marker | — |
| G-042 | Focused and practical complete non-live test suites | proved | Current focused release/privacy/public-link tests, complete deterministic non-live unit suite and bounded relevant UI matrix pass; the two named live-provider tests remain excluded and zero live traffic occurred | — |
| G-043 | Debug, Staging and Release Simulator builds | proved | Debug, Staging and Release generic Simulator configurations pass on the audited baseline using one task-owned DerivedData path | — |
...
| G-050 | Final evidence manifest on integrated source | proved | Listed application/configuration blobs were rehashed at exact reachable commit `21f8450c976252210edf03389dc1b682d2440450`; current package-tooling hashes are separately bound and historical execution is not relabeled | — |
