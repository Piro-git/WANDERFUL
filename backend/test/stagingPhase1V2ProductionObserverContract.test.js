...
describe("staging Phase 1 V2 production observer acceptance contract", () => {
  it("keeps production admission unreachable while provider proofs or trust anchors are absent", () => {
    assert.throws(
      () => assertStagingPhase1V2ProductionObserverCapabilities(),
  ...
  it("pins four chained durable files and consumes OAuth only into session memory", () => {
    assert.deepEqual(STAGING_PHASE1_V2_PRODUCTION_ARTIFACT_MANIFEST.phases, [
      ...
      { sequence: 2, phase: "post-ddl-advisors",
        suffix: "observer.02.post-ddl-advisors.json" },
      { sequence: 3, phase: "final-control",
        suffix: "observer.03.final-control.json" },
      { sequence: 4, phase: "post-disconnect-cleanup",
        suffix: "observer.04.post-disconnect-cleanup.json" }
    ]);
    ...
    assert.match(artifacts,
      /signatureDomain:\s*"trailmind-production-observer-v1"/);
    assert.match(artifacts,
...
function cleanupSamples(binding) {
  const sample = (observedAt, statsSnapshotId) => ({
    ...binding,
    clearSnapshot: true,
  ...
  return [
    sample("2026-08-29T10:00:00.000Z", randomUUID()),
    sample("2026-08-29T10:00:00.250Z", randomUUID())
  ];
}
