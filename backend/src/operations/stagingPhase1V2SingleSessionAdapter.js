...
function createSession({
  ...
  async function run() {
    return runStagingPhase1V2Operator({
      ...
      approval: {
        ...
        operatorDigestsDigest: canonicalAclDigest(admission.operatorDigests),
  ...
  function executorAdapter() {
    return {
      ...
      async compensatePreLedger({ lock }) {
        await attestLocked(lock);
        await query(
  ...
  async function finalize({ staged, cleanupVerifier, receiptStore }) {
    ...
    if (
      !isExactObject(cleanupProof, [
        "backendSessionCount", "evidenceDigest", "idleSessionCount",
        "observedAt"
      ]) || cleanupProof.backendSessionCount !== 0 ||
      cleanupProof.idleSessionCount !== 0 ||
      typeof cleanupProof.observedAt !== "string" ||
    ...
    if (
      !isExactObject(persistence, [
        "evidenceDigest", "ordinal", "phase", "receiptBytes",
        "receiptDigest", "status"
