...
describe("production observer auditor on disposable PostgreSQL 17", {
  ...
  it("proves exact least privilege, run binding, TLS and two-sample cleanup", async () => {
    ...
    const auditor = new pg.Client({
      ...connection,
      application_name: "trailmind_p1v2_auditor_disposable",
    ...
    try {
      ...
      assert.throws(() => validateStagingPhase1V2CleanupSamples([
        cleanupSample(expected, stillActive),
        cleanupSample(expected, stillActive)
      ], expected));
      await auditor.query("ROLLBACK");
      ...
      for (let index = 0; index < 2; index += 1) {
        ...
        await auditor.query("SET LOCAL stats_fetch_consistency = 'none'");
        await auditor.query("SELECT pg_catalog.pg_stat_clear_snapshot()");
        const sample = (await auditor.query(
          stagingPhase1V2ProductionAuditorStatement("target_session_v1"),
          [expected.backendPid, expected.applicationName,
            expected.backendStart]
        )).rows[0];
        samples.push(cleanupSample(expected, sample));
        await auditor.query("ROLLBACK");
        if (index === 0) {
...
function cleanupSample(expected, row) {
  return {
    ...expected,
    clearSnapshot: true,
    exactBackendInstanceCount: row.exact_backend_instance_count,
    idleExactInstanceCount: row.idle_exact_instance_count,
    matchingApplicationCount: row.matching_application_count,
    observedAt: new Date().toISOString(),
    samePidOtherInstanceCount: row.same_pid_other_instance_count,
