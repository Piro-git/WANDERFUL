...
WITH anchor AS (
  ...
  SELECT projection.entity_id,
   ...
   WHERE projection.projection_run_id = $1
     ...
     AND projection.projected_geometry &&
         ST_Expand(anchor.point, $6::double precision / 111000.0)
     AND ST_DWithin(
       ST_PointOnSurface(projection.projected_geometry)::geography,
       anchor.point::geography,
       $6::double precision
...
WITH anchor AS (
  ...
  SELECT relationship.relationship_id,
   ...
   WHERE projected_relationship.projection_run_id = $1
     ...
     AND segment.projected_geometry &&
         ST_Expand(anchor.point, $5::double precision / 111000.0)
     AND ST_DWithin(
       ST_PointOnSurface(segment.projected_geometry)::geography,
       anchor.point::geography,
       $5::double precision
...
function minimumFreshnessMilliseconds(row, policy) {
  ...
  const expectedRefreshSeconds = Number(row.expected_refresh_interval_seconds);
  const values = days.map((value) => value * 86_400_000);
  if (Number.isInteger(expectedRefreshSeconds) && expectedRefreshSeconds > 0) {
    values.push(expectedRefreshSeconds * 1_000);
  }
