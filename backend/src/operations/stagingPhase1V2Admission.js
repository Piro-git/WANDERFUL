...
const KNOWN_MIGRATION_FILES = Object.freeze([
  ...
  SUPABASE_POSTGIS_ISOLATION_MIGRATIONS_V2[7]
]);
...
export function admitStagingPhase1V2Session(request, dependencies = {}) {
  ...
  try {
    const consumedEnvelope = consumeAuthorizationEnvelope({
      path: request.authorizationEnvelopePath,
      root,
      io
    });
    envelope = consumedEnvelope.envelope;
    authorizationEnvelopeDigest = consumedEnvelope.sha256;
...
function consumeAuthorizationEnvelope({ path, root, io }) {
  assertOutsideRepository(path, root, io, "authorization");
  let initial;
  try {
    initial = io.lstat(path);
  } catch {
    blocked("authorization_file");
  }
  assertProtectedRegularFile(initial, "authorization_file", { singleLink: true });
  if (initial.size === 0 || initial.size > MAXIMUM_AUTHORIZATION_BYTES) {
    blocked("authorization_size");
  }
  const claimedPath = `${path}.consuming-${process.pid}-${randomUUID()}`;
  let claimed = false;
  try {
    io.rename(path, claimedPath);
    claimed = true;
    const after = io.lstat(claimedPath);
    assertProtectedRegularFile(after, "authorization_file", { singleLink: true });
    if (after.dev !== initial.dev || after.ino !== initial.ino) {
      blocked("authorization_race");
    }
    const bytes = io.readFile(claimedPath);
    ...
    if (claimed) {
      try { io.unlink(claimedPath); } catch { /* already consumed */ }
    }
  }
}
...
function assertCandidateGit(value, root) {
  if (
    !isExactObject(value, ["baselineReachable", "clean", "head", "root"]) ||
    value.baselineReachable !== true || value.clean !== true ||
    !COMMIT_PATTERN.test(value.head) || resolve(value.root) !== root
  ) blocked("candidate_git");
}
